...
public class SlotUtils {
    ...
    public static @NotNull Set<Integer> getSlotsRow(int row, int width) {
        Set<Integer> slots = new LinkedHashSet<>();
        for (int x = 0; x < width; x++) slots.add(convertToIndex(x, row, width));
        return slots;
    }

    public static @NotNull Set<Integer> getSlotsColumn(int column, int width, int height) {
        Set<Integer> slots = new LinkedHashSet<>();
        for (int y = 0; y < height; y++) slots.add(convertToIndex(column, y, width));
        return slots;
    }

    public static @NotNull Set<Integer> getSlotsBorders(int width, int height) {
        Set<Integer> slots = new LinkedHashSet<>();
    ...
    public static @NotNull Set<Integer> getSlotsRect(int x, int y, int width, int height, int frameWidth) {
        return getSlotsRect(Order.HORIZONTAL, x, y, width, height, frameWidth);
    }

    public static @NotNull Set<Integer> getSlotsRect(@NotNull Order order, int x, int y, int width, int height, int frameWidth) {
        Set<Integer> slots = new LinkedHashSet<>();
    ...
    public static @NotNull Point2D convertFromIndex(int index, int width) {
