...
import kr.starly.libs.inventory.inventory.ObscuredInventory;
...
import kr.starly.libs.inventory.item.impl.controlitem.ControlItem;
import kr.starly.libs.inventory.window.*;
...
public abstract class AbstractGui implements Gui, GuiParent {
            ...
            if (inventory.isSynced(slot, clicked) || didClickBackgroundItem(player, element, inventory, slot, clicked)) {
                ...
                switch (event.getClick().name()) {
                    ...
                    case "DOUBLE_CLICK":
                        ...
                        break;
                    default:
                        InventoryFramework.getLogger().warning("Unknown click type: " + event.getClick().name());
                        break;
                }
            }
        }
    ...
    private boolean didClickBackgroundItem(Player player, SlotElement.InventorySlotElement element, Inventory inventory, int slot, ItemStack clicked) {
        return !inventory.hasItem(slot) && (isBuilderSimilar(background, clicked) || isBuilderSimilar(element.getBackground(), clicked));
    }

    private boolean isBuilderSimilar(ItemProvider builder, ItemStack expected) {
        return builder != null && builder.get().isSimilar(expected);
    }
